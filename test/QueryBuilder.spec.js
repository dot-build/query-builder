...
describe('QueryBuilderOperators', function() {
    ...
    describe('#select()', function() {
        it('should allow the input of field names as an array of strings', function () {
            var instance = QueryBuilder.create();
            expect(instance.$$select).toEqual([]);

            var chain = instance.select(['foo', 'bar']);
            expect(instance.$$select).toEqual(['foo', 'bar']);
            expect(chain).toBe(instance);
        });

        it('should allow the input of a prefix and field names as an array of strings that are appended to prefix with a DOT as separator', function () {
            var instance = QueryBuilder.create();
            expect(instance.$$select).toEqual([]);

            var chain = instance.select('prefix', ['foo', 'bar']);
            expect(instance.$$select).toEqual(['prefix.foo', 'prefix.bar']);
            expect(chain).toBe(instance);
        });

        it('should allow a single string parameter as a field name to select', function () {
            var instance = QueryBuilder.create();
            expect(instance.$$select).toEqual([]);

            var chain = instance.select('foo.name');
            expect(instance.$$select).toEqual(['foo.name']);
            expect(chain).toBe(instance);
        });

        it('should not allow values other than arrays as fields', function () {
            var instance = QueryBuilder.create();
